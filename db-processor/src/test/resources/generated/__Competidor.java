...
abstract class __Competidor extends ChildWithId {
  ...
  final Id personId = new Id(canonicalName("processor.test.Competidor.personId"), description("Person Id"), fieldName("personId"),
                             jsonName("personId"), jsonPath("personId"), noConstraints);

  final Parent<Competencia> competencia = new Parent(Competencia.m, canonicalName("processor.test.Competidor.competencia"), description("Competencia"),
                                                     fieldName("competencia"), nullable, jsonName("competencia"), jsonPath("competencia"),
                                                     noConstraints);

  final IndexedString nombreVisible = new IndexedString(canonicalName("processor.test.Competidor.nombreVisible"), description("Nombre Visible"),
                                                        propertyName("nombreVisible"), fieldName("nombreVisible"), nullable, jsonName("nombreVisible"),
                                                        jsonPath("nombreVisible"), constraints(ae.db.NotBlankConstraint.ForString.INSTANCE));

  final IndexedString nombres = new IndexedString(canonicalName("processor.test.Competidor.nombres"), description("Nombres"), propertyName("nombres"),
                                                  fieldName("nombres"), required, jsonName("nombres"), jsonPath("nombres"), constraints(
                                                  ae.db.NotBlankConstraint.ForString.INSTANCE));

  final IndexedString apellidos = new IndexedString(canonicalName("processor.test.Competidor.apellidos"), description("Apellidos"), propertyName(
                                                    "apellidos"), fieldName("apellidos"), required, jsonName("apellidos"), jsonPath("apellidos"),
                                                    constraints(ae.db.NotBlankConstraint.ForString.INSTANCE));

  final IndexedString prefijo = new IndexedString(canonicalName("processor.test.Competidor.prefijo"), description("Prefijo"), propertyName("prefijo"),
                                                  fieldName("prefijo"), nullable, jsonName("prefijo"), jsonPath("prefijo"), noConstraints);

  final IndexedString sufijo = new IndexedString(canonicalName("processor.test.Competidor.sufijo"), description("Sufijo"), propertyName("sufijo"),
                                                 fieldName("sufijo"), nullable, jsonName("sufijo"), jsonPath("sufijo"), noConstraints);

  final IndexedString apodo = new IndexedString(canonicalName("processor.test.Competidor.apodo"), description("Apodo"), propertyName("apodo"),
                                                fieldName("apodo"), nullable, jsonName("apodo"), jsonPath("apodo"), constraints(
                                                ae.db.NotBlankConstraint.ForString.INSTANCE));

  final IndexedDate nacimiento = new IndexedDate(canonicalName("processor.test.Competidor.nacimiento"), description("Nacimiento"), propertyName(
                                                 "nacimiento"), fieldName("nacimiento"), required, jsonName("nacimiento"), jsonPath("nacimiento"),
                                                 new DateJsonSerializer("yyyy-MM-dd"), noConstraints);

  final IndexedString sexo = new IndexedString(canonicalName("processor.test.Competidor.sexo"), description("Sexo"), propertyName("sexo"), fieldName(
                                               "sexo"), required, jsonName("sexo"), jsonPath("sexo"), noConstraints);

  final IndexedPhoneNumber telefonoPersonal = new IndexedPhoneNumber(canonicalName("processor.test.Competidor.telefonoPersonal"), description(
                                                                     "Telefono"), propertyName("fono"), fieldName("telefonoPersonal"), nullable,
                                                                     jsonName("telefonoPersonal"), jsonPath("telefonoPersonal"), noConstraints);

  final IndexedPhoneNumber telefonoEmergencias = new IndexedPhoneNumber(canonicalName("processor.test.Competidor.telefonoEmergencias"), description(
                                                                        "Telefono de Emergencia"), propertyName("emergencia"), fieldName(
                                                                        "telefonoEmergencias"), nullable, jsonName("telefonoEmergencias"), jsonPath(
                                                                        "telefonoEmergencias"), noConstraints);

  final IndexedEmail email = new IndexedEmail(canonicalName("processor.test.Competidor.email"), description("Email"), propertyName("email"),
                                              fieldName("email"), required, jsonName("email"), jsonPath("email"), noConstraints);

  final IndexedEmail emailEmergencias = new IndexedEmail(canonicalName("processor.test.Competidor.emailEmergencias"), description("Email Emergencias"),
                                                         propertyName("emailEmergencias"), fieldName("emailEmergencias"), nullable, jsonName(
                                                         "emailEmergencias"), jsonPath("emailEmergencias"), noConstraints);

  final UnindexedText info = new UnindexedText(canonicalName("processor.test.Competidor.info"), description("Info"), propertyName("info"), fieldName(
                                               "info"), nullable, jsonName("info"), jsonPath("info"), constraints(
                                               ae.db.NotBlankConstraint.ForText.INSTANCE));

  final IndexedKeyList participaciones = new IndexedKeyList(canonicalName("processor.test.Competidor.participaciones"), description("Participaciones"),
                                                            propertyName("participaciones"), fieldName("participaciones"), nullable, jsonName(
                                                            "participaciones"), jsonPath("participaciones"), noConstraints);

  private final ImmutableList<Attr> _attrs = ImmutableList.of(personId, competencia, nombreVisible, nombres, apellidos, prefijo, sufijo, apodo,
                                                              nacimiento, sexo, telefonoPersonal, telefonoEmergencias, email, emailEmergencias, info,
                                                              participaciones);

  private final ImmutableList<Field<?>> _fields = ImmutableList.of(nombreVisible, nombres, apellidos, prefijo, sufijo, apodo, nacimiento, sexo,
                                                                   telefonoPersonal, telefonoEmergencias, email, emailEmergencias, info,
                                                                   participaciones);
  ...
  public JsonNode toJson(final Entity data)
  {
    ...
    return JsonNodeFactories.object(ImmutableList.of(personId.makeJsonFieldFrom(data), nombreVisible.makeJsonFieldFrom(data), nombres.
                                                     makeJsonFieldFrom(data), apellidos.makeJsonFieldFrom(data), prefijo.makeJsonFieldFrom(data),
                                                     sufijo.makeJsonFieldFrom(data), apodo.makeJsonFieldFrom(data), nacimiento.makeJsonFieldFrom(data),
                                                     sexo.makeJsonFieldFrom(data), telefonoPersonal.makeJsonFieldFrom(data), telefonoEmergencias.
                                                     makeJsonFieldFrom(data), email.makeJsonFieldFrom(data), emailEmergencias.makeJsonFieldFrom(data),
                                                     info.makeJsonFieldFrom(data), participaciones.makeJsonFieldFrom(data)));
  }
