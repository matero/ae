...
abstract class RouterDefs extends RouterServlet {
    ...
    private final ParameterizedRoute GET_book_Controller_show = new ParameterizedRoute("/book/${id}", Pattern.compile(
                                                                                       "/book/$(?<p0>[^/]+)"));

    private final ParameterizedRoute GET_book_Controller_edit = new ParameterizedRoute("/book/${id}/edit", Pattern.
                                                                                       compile("/book/$(?<p0>[^/]+)/edit"));

    private final ParameterizedRoute GET_book_Controller_foo = new ParameterizedRoute("/book/${id}/{$arg}", Pattern.
                                                                                      compile("/book/$(?<p0>[^/]+)/(?<p1>[^/]+)"));

    private final ParameterizedRoute GET_book_Controller_bar = new ParameterizedRoute("/book/${id}/{$c}/{$arg}",
                                                                                      Pattern.compile(
                                                                                              "/book/$(?<p0>[^/]+)/(?<p1>[^/]+)/(?<p2>[^/]+)"));
    ...
    private final ParameterizedRoute GET_gym_Controller_show = new ParameterizedRoute("/gym/${id}", Pattern.compile(
                                                                                      "/gym/$(?<p0>[^/]+)"));

    private final ParameterizedRoute GET_gym_Controller_edit = new ParameterizedRoute("/gym/${id}/edit", Pattern.
                                                                                      compile("/gym/$(?<p0>[^/]+)/edit"));
    ...
    private final ParameterizedRoute PUT_book_Controller_update = new ParameterizedRoute("/book/${id}", Pattern.compile(
                                                                                         "/book/$(?<p0>[^/]+)"));

    private final ParameterizedRoute PUT_gym_Controller_update = new ParameterizedRoute("/gym/${id}", Pattern.compile(
                                                                                        "/gym/$(?<p0>[^/]+)"));

    private final ParameterizedRoute DELETE_book_Controller_delete = new ParameterizedRoute("/book/${id}", Pattern.
                                                                                            compile("/book/$(?<p0>[^/]+)"));

    private final ParameterizedRoute DELETE_gym_Controller_delete = new ParameterizedRoute("/gym/${id}", Pattern.
                                                                                           compile("/gym/$(?<p0>[^/]+)"));
            ...
            ServletException, IOException
    {
        ...
        if (GET_book_Controller_htmlIndex.matches(request)) {
            if ("text/html".equals(request.getHeader("Accept"))) {
                handle(new Controller_Impl(request, response, webContext(request, response), templateEngine()),
                       (controller) -> controller.htmlIndex());
                return;
            }
        ...
        if (GET_client_Controller_htmlIndex.matches(request)) {
            if ("text/html".equals(request.getHeader("Accept"))) {
                handle(new client.Controller_Impl(request, response, webContext(request, response), templateEngine()),
                       (controller) -> controller.htmlIndex());
                return;
            }
        ...
        if (GET_gym_Controller_htmlIndex.matches(request)) {
            if ("text/html".equals(request.getHeader("Accept"))) {
                handle(new gym.Controller_Impl(request, response, webContext(request, response), templateEngine()),
                       (controller) -> controller.htmlIndex());
                return;
            }
